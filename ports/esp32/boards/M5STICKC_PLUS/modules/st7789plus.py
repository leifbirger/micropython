...
class ST7789PLUS(ST7789):
    ...
    def __init__(self):
        ...
        dcp=Pin(23, Pin.OUT)
        csp=Pin(5, Pin.OUT)
        spi = SPI(baudrate=4000000,sck=Pin(13), mosi=Pin(15), miso=Pin(4), phase=1, polarity=1)   
        #super().__init__(spi, ROWS, COLUMNS, reset=Pin(18, Pin.OUT), dc=Pin(23, Pin.OUT), cs=Pin(5, Pin.OUT), rotation=3)
        ST7789.__init__(self, spi, ROWS, COLUMNS, reset=Pin(18, Pin.OUT), dc=Pin(23, Pin.OUT), cs=Pin(5, Pin.OUT), rotation=3)
        #self.init()
        #self.testpicture()
        #self.testtext()
        print("end of test")
