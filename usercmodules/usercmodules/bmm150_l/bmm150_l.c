...
#define ENABLE_DEBUG2
#ifdef ENABLE_DEBUG2
    #define dprintf(...) if(dev->debug) printf(__VA_ARGS__)
#else
    #define dprintf(...) /**/
#endif
...
STATIC int read_i2c(mp_obj_t i2c_obj, uint16_t addr, uint32_t memaddr) { 
     ...
     read_mem(i2c_obj, addr, memaddr, memaddrsize, &value_local, (size_t)1);
	 XDEBUG("read i2c: %x  data: %x \n", memaddr, (unsigned int)value_local);
...
uint8_t bmm150_read_reg(const bmm150_context dev, uint8_t reg)
{
	 ...
	 dprintf("read reg: %x  data: %x \n", reg, (unsigned int)value_local);
                     ...
                     uint8_t *buffer, int len)
{
    ...
    for (int i=0; i<len; i++) {
		...
		dprintf("regs buffer: %x  data: %x \n", (reg+i), (unsigned int)buffer[i]);
	}
...
upm_result_t  bmm150_init(bmm150_context dev)
{
    ...
    if (chipID != BMM150_DEFAULT_CHIPID)
    {
		...
		mp_raise_msg(&mp_type_RuntimeError, "BMM150: Wrong Chip ID");
        assert(false);
        return UPM_ERROR_OPERATION_FAILED;
    }
	...
	if (bmm150_devinit(dev, BMM150_USAGE_HIGH_ACCURACY))
    {
        ...
        //bmm150_close(dev);
		mp_raise_msg(&mp_type_RuntimeError, "BMM150: Init failed");
        assert(false);
        return UPM_ERROR_OPERATION_FAILED;
    }
...
upm_result_t bmm150_update(const bmm150_context dev)
{
	...
	dprintf("1up.hall.buf.7o6: %x : %x \n", (unsigned int)buf[7]),  (unsigned int)buf[6]);
	dprintf("1up.hall: %u \n", (unsigned int)dev->hall);
	dprintf("1up.hall: %x \n", (unsigned int)dev->hall);
    dev->hall /= 4;
	dprintf("2up.hall.buf.7o6: %x : %x \n", (unsigned int)buf[7], (unsigned int)buf[6]);
	dprintf("2up.hall: %u \n", (unsigned int)dev->hall);
	dprintf("2up.hall: %x \n", (unsigned int)dev->hall);
...
STATIC mp_obj_t bmm150_BMM150_read_reg(mp_obj_t self_in, mp_obj_t memaddr_obj) {
     bmm150_BMM150_obj_t *self = MP_OBJ_TO_PTR(self_in);
     uint32_t memaddr = (uint32_t)mp_obj_get_int(memaddr_obj); 
	 // Call C function
	 int result =  bmm150_read_reg( &(self->dev), (uint8_t)memaddr);
     return mp_obj_new_int(result);
}
// Define a Python reference to the function above.
STATIC MP_DEFINE_CONST_FUN_OBJ_2(bmm150_BMM150_read_reg_obj, bmm150_BMM150_read_reg);


STATIC mp_obj_t bmm150_BMM150_read_regs(mp_obj_t self_in, mp_obj_t memaddr_obj, mp_obj_t len_obj) {
     bmm150_BMM150_obj_t *self = MP_OBJ_TO_PTR(self_in);
     uint32_t memaddr = (uint32_t)mp_obj_get_int(memaddr_obj); 
     uint32_t len = (uint32_t)mp_obj_get_int(len_obj);
     uint8_t buffer[len];
	 int len_return;
 	 // Call C function
	 len_return = bmm150_read_regs( &(self->dev), (uint8_t)memaddr, buffer, len);
	 
	 mp_obj_t tuple[len]; 
	 for (int i=0;i<len;i++) tuple[i] = mp_obj_new_int(buffer[i]);
     return mp_obj_new_tuple(len, tuple);
}
// Define a Python reference to the function above.
STATIC MP_DEFINE_CONST_FUN_OBJ_3(bmm150_BMM150_read_regs_obj, bmm150_BMM150_read_regs);


STATIC mp_obj_t bmm150_BMM150_write_reg(size_t n_args, const mp_obj_t *args) {
     bmm150_BMM150_obj_t *self = MP_OBJ_TO_PTR(args[0]);
     uint32_t memaddr = (uint32_t)mp_obj_get_int(args[1]);
     uint8_t value =(uint8_t)mp_obj_get_int(args[2]);
	 // Call C function
	 bmm150_write_reg( &(self->dev ), (uint8_t)memaddr, (uint8_t)value);
     return mp_const_none;
}
// Define a Python reference to the function above.
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(bmm150_BMM150_write_reg_obj, 3, 3, bmm150_BMM150_write_reg);
...
STATIC mp_obj_t bmm150_BMM150_update(mp_obj_t self_in) {
    ...
    // Call C function
	if(bmm150_update( &(self->dev) ))
	        printf("%s: bmm150_update() failed.\n", __FUNCTION__);
    return mp_const_none;
}
// Define a Python reference to the function above.
...
STATIC const mp_rom_map_elem_t bmm150_BMM150_locals_dict_table[] = {
    ...
    { MP_ROM_QSTR(MP_QSTR_read_reg),   MP_ROM_PTR(&bmm150_BMM150_read_reg_obj) },
    { MP_ROM_QSTR(MP_QSTR_read_regs),  MP_ROM_PTR(&bmm150_BMM150_read_regs_obj) },
    { MP_ROM_QSTR(MP_QSTR_write_reg),  MP_ROM_PTR(&bmm150_BMM150_write_reg_obj) },
...
//  mp_obj_base_t *spi_obj = (mp_obj_base_t*)MP_OBJ_TO_PTR(args[ARG_spi].u_obj);
	...
	if(n_args > 2) {
		self->debug = mp_obj_get_int(args[2]);
		self->dev.debug = self->debug;
	}
	if(bmm150_init( &(self->dev) ))
	        printf("%s: bmm150_init() failed.\n", __FUNCTION__);
