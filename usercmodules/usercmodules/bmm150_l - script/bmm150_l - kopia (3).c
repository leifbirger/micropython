...
STATIC mp_obj_t bmm150_read_byte(mp_obj_t i2c_obj, mp_obj_t addr_obj, mp_obj_t memaddr_obj) {
     ...
     uint8_t value;
     read_mem(i2c_obj, addr, memaddr, (uint8_t)1, &value, (size_t)1);
...
STATIC mp_obj_t bmm150_write_byte(mp_obj_t i2c_obj, mp_obj_t addr_obj, mp_obj_t memaddr_obj, mp_obj_t value_obj) {
     uint16_t addr = (uint16_t)mp_obj_get_int(addr_obj);
     uint32_t memaddr = (uint32_t)mp_obj_get_int(memaddr_obj);       
     uint8_t value =(uint8_t)mp_obj_get_int(value_obj);
	 write_mem(i2c_obj, addr, memaddr, (uint8_t)1, &value, (size_t)1);
     return mp_const_none;
}
// Define a Python reference to the function above.
STATIC MP_DEFINE_CONST_FUN_OBJ_4(bmm150_write_byte_obj, bmm150_write_byte);
